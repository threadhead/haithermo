...
class TestThermostat < Test::Unit::TestCase
  ...
  def test_add_thermostat_check_address_range
    assert_raise(HAIthermo::ThermostatError) {@control.add_thermostat(888) }
    assert_raise(HAIthermo::ThermostatError) {@control.add_thermostat(128) }
    assert_raise(HAIthermo::ThermostatError) {@control.add_thermostat(0) }
    assert_nothing_raised(HAIthermo::ThermostatError) { @control.add_thermostat(1) }
    assert_nothing_raised(HAIthermo::ThermostatError) { @control.add_thermostat(64) }
    assert_nothing_raised(HAIthermo::ThermostatError) { @control.add_thermostat(127) }
  end
